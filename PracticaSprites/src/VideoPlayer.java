...
public class VideoPlayer extends JFrame {
	...
	public VideoPlayer() {
		...
		bAnyadir.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ficheros = JOptionPane.showInputDialog(null, "ficheros a elegir", "seleccion", JOptionPane.QUESTION_MESSAGE);
	...
	private void lanzaVideo() {
		if (mediaPlayerComponent.getMediaPlayer()!=null &&
			...
			fechita = new Date( ficVideo.lastModified() );
			lMensaje.setText("Fecha: " + fechita.toString());
			lMensaje.repaint();
	...
	private static File pedirCarpeta() {
		...
		chooser.setFileSelectionMode( JFileChooser.DIRECTORIES_ONLY ); // solo directorios :S
		int v = chooser.showOpenDialog( null );
		if(v == JFileChooser.APPROVE_OPTION)  { //como no puedo comprobar he encontrao esta manera en internet que parece que tiene que funcinar
			return f; 
		}
		else {
		return null;
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			ficheros = JOptionPane.showInputDialog(null, "ficheros a elegir", "seleccion", JOptionPane.QUESTION_MESSAGE);
			//creo que haciendo esto saltarian para seleccionar ficheros. 
    	...
    	if (!found) {
			...
			if (vlcPath==null) {  // Poner VLC a mano
	        	System.setProperty("jna.library.path", "c://Applicaciones/vlc");
