...
public class ListaDeReproduccion implements ListModel<String> {
	...
	private static Logger logger = Logger.getLogger( ListaDeReproduccion.class.getName()); //no me he enterado muy bien del logger :(
	...
	public int add(String carpetaFicheros, String filtroFicheros) {
		...
		File carpeta = new File(carpetaFicheros); //no tengo forma de controlar que este funcione aparte de las pruebas con JUnit y consola
		//porque me da errores al iniciar el VLC. 
		for(final File fich : carpeta.listFiles()) {
			//fich.getName(); 
			Matcher m = p.matcher(fich.getName());
			if(m.matches()) {
				ficherosLista.add(fich);
				logger.log(Level.INFO, "fichero añadido");
				contador = contador +1; 
	...
	public void removeFich(int num) {
		if(ficherosLista.size() >= num) {
			ficherosLista.remove(num);
		}
	}
	...
	public void intercambia(int pos1 , int pos2) {
		File f = ficherosLista.get(pos1);
		File l = ficherosLista.get(pos2);
		
		ficherosLista.set(pos1, l);
		ficherosLista.set(pos2, f);
	}
	
	//no se deberia crear abajo pero crearemos aqui el constructor, iniciando una lista vacia
	
	public ListaDeReproduccion() {
		ficherosLista = new ArrayList<>();
	}
	
	/** Añade a la lista de reproducción todos los ficheros que haya en la carpeta indicada, que * cumplan el filtro indicado. Si hay cualquier error, la lista de reproducción queda solo con * los ficheros que hayan podido ser cargados de forma correcta.
	* @param carpetaFicheros
	* @param filtroFicheros
	*
	*
	Path de la carpeta donde buscar los ficheros
	Filtro del formato que tienen que tener los nombres de los ficheros
	para ser cargados. String con cualquier letra o dígito. Si tiene un
	asterisco hace referencia a cualquier conjunto de letras o dígitos.
	Por ejemplo p*.* hace referencia a cualquier fichero de nombre que
	empiece por p y tenga cualquier extensión.
	*
	*
	* @return Número de ficheros que han sido añadidos a la lista */
	public int add1( String carpetaFicheros, String filtroFicheros ) {
	...
	public Random randomEs = new Random();
	...
	public boolean irARandom(){
		...
		ficheroEnCurso = randomEs.nextInt( ficherosLista.size() );
